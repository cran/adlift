    ...
    if (!closest) {
        ...
        if (is.element(r, range)) {
            distances <- matrix(0, 1, leftneigh + rightneigh)
                distances <- abs(X[remove] - X[pointsin[r - 
                  leftneigh + (1:leftneigh) - 1]])
                distances[(1:rightneigh) + leftneigh] <- abs(X[remove] - X[pointsin[r + 
                  (1:rightneigh)]])
            d1 <- min(d, N - 1)
            q <- order(distances)[1:d1]
            nbrs <- nbrs[q]
            index <- setdiff((r - leftneigh):(r + rightneigh), 
                r)
            index <- index[q]
            B <- matrix(0, 1, d1)
                B <- (index[q[1:d1]] < r)
