...
X<-X$X
    ...
    if (nadd > 0) {
        for (j in 1:nadd) {
            ...
            if (nt == length(nbrs)) {
                r <- which(pointsin == nbrs[length(nbrs)]) + 
                  1
            }
            ...
            if (is.null(schemehist) == FALSE) {
                ...
                res <- LocalPred(pointsin, X, coeff, nbrs, remove, 
                  intercept, neighbours)
            }
            if (length(res) == 2) {
                l <- res[[1]]
            }
            ...
            l1 <- UndoPointsUpdate(X, coeff, nbrs, index, remove, 
                r, N, pointsin, gamweights, lengths, lengthrem)
            coeff <- l1$coeff
            lengths <- l1$lengths
            alpha <- l1$alpha
    ...
    return(list(X = X, coeff = coeff, lengths = lengths, lengthsremove = lengthsremove, 
