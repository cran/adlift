...
function (y, detail, al) 
{
    ...
    while (sum(ch) != 0) {
        ...
        for (i in 1:length(detail)) {
            idlist[[i]] <- which((y >= ep[i, 1]) & (y <= ep[i, 
                2]))
            newidlist[[i]] <- idlist[[i]][is.element(idlist[[i]], 
                al[[1]])]
    ...
    return(list(ep1 = ep1, ep2 = ep2, idlist = idlist, newidlist = newidlist, 
        dlist = dlist, varvec = varvec, varvec1 = varvec1, varvec2 = varvec2))
}
