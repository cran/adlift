`denoiseheteroprop2` <-function (x, f, pred=1, neigh=1, int=TRUE, clo=FALSE, keep=2, rule = "median", gamvec=rep(1,times=length(x)), verbose=TRUE,index=1:length(x)) 
{
    newcoeff <- NULL
    ndetlist <- list()
    tclist <- list()
    ...
    n<-length(x)
    temp <- fwtnp2(x, f, pred, neigh, int, clo, keep,do.W=TRUE,varonly=FALSE)
    w <- temp$Wnew
    out <- temp$out
    ...
    return(list(out = out, w = w, indsd = indsd, al = al, sd = sd, 
        fhat = fhat))
}
